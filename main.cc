...
int main(){
	...
	//SFML OBJECTS
	sf::Event event;
	sf::Clock deltaClock, melocClock, zumoClock, pokeClock, spawnClock;
	sf::Image image;
	sf::Sprite player, background, meloc, zumo, stone, pokeball, zreza;
	sf::Texture tplayer, tbackground, tmeloc, tzumo, tstone, tpokeball, tzreza;
	...
	//VARIABLES
	int melocCount = 0;
	bool speedup = false, raichu = false, poke=false;
	direction d = none;
	bool keypressed = false;
	float deltatime = 0, movx = 0.0, movy = 0.0, scont = 0.0;
	float distx = spriteSize.x/4, disty = spriteSize.y/4;
	...
	while(window.isOpen()){
		...
		for (int i = 0; i < bayas.size(); ++i){
			if(player.getGlobalBounds().intersects(bayas[i].second.getGlobalBounds())){
